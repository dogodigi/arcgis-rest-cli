 ...
 */
exports.postRetrieve = function (url, postdata, type, callback) {
    var http;
 ...
 * @returns {object} JSON object
 */
exports.retrieve = function (urlpath, type, callback) {
    var http;
    if (this.config && this.config.useTor) {
        http = require('socks5-http-client');
    } else {
...
exports.getIdfield = function (service, layer, callback) {
    ...
    this.retrieve(fieldUrl, '', function (result) {
        if (result) {
...
exports.getObjectIds = function (service, layer, callback) {
    ...
    this.postRetrieve(objectidUrl, postdata, '', function (result) {
        if (result) {
            callback(result);
        } else {
            callback([]);
 ...
 * @return {array} result
 */
exports.processLists = function (callback) {
    var _obj = this;
    var listurl = this.config.arcGISRootQuery || '/ArcGIS/rest/services/?f=json&pretty=true';
    var useServices = this.config.useRootServices || true;
    this.retrieve(listurl, '', function (listresult) {
        if (listresult) {
            var resultlist = {
                "services": [],
                "folders": []
            ...
            if (listresult.folders) {
                if (!_obj.config.useFolderList) {
                    resultlist.folders = listresult.folders;
                } else {
                    if (!_obj.config.folderList) {
                        resultlist.folders = [];
                    } else {
                        resultlist.folders = _obj.intersect(_obj.config.folderList, listresult.folders);
                    }
                }
            } else {
                _obj.logger('info', 'No folders for ' + _obj.config.options.hostname + '' + listurl);
            }
            ...
            if (listresult.services) {
                ...
                if (useServices) {
                    ...
                    if (!_obj.config.useServicesList) {
                        resultlist.services = newServicesList;
                    } else {
                        if (!_obj.config.servicesList) {
                            resultlist.folders = [];
                        } else {
                            resultlist.services = intersect(_obj.config.servicesList, newServicesList);
                        }

                    }
                }
            } else {
                _obj.logger('info', 'No services for ' + _obj.config.options.hostname + '' + listurl);
            }
            callback(resultlist);
        } else {
            _obj.logger('info', 'Incorrect response for ' + _obj.config.options.hostname + '' + listurl);
            callback();
        }
    });
};
